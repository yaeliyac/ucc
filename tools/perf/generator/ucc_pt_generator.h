...
private:
    ...
    ucc_pt_op_type_t op_type;
public:
    ucc_pt_generator_traffic_matrix(int kind, uint32_t gsize, uint32_t rank, ucc_datatype_t dtype, ucc_pt_op_type_t type, size_t nrep, int token_size_KB_mean, int num_tokens, int tgt_group_size_mean);
    bool has_next() override;
    void next() override;
    void reset() override;
    size_t get_src_count() override;
