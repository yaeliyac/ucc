...
std::vector<std::vector<int>> create_a2aV_traffic_matrix(int num_ranks, int token_size_KB_mean, int tgt_group_size_mean,
                                                         int num_tokens, size_t dt_size, std::mt19937_64 &rng,
                                                         bool add_bias = false, double bias_factor = 2,
                                                         int num_hl_ranks = 2) {
    ...
    std::vector<std::vector<int>> traffic_matrix(
        num_ranks, std::vector<int>(num_ranks, 0)); // matrix of size num_ranks x num_ranks
    for (int i = 0; i < num_hl_ranks; i++) {
        bias_indices[i] = std::uniform_int_distribution<int>(0, num_ranks - 1)(rng);
    }
        ...
        for (int token = 0; token < num_tokens; token++) {
            ...
            if (add_bias) {
                ...
                for (int i = 0; i < num_hl_ranks; i++) {
                    ...
                    auto it        = std::find(possible_targets.begin(), possible_targets.end(), bias_rank);
                    if (it != possible_targets.end()) {
                        int bias_index = std::distance(possible_targets.begin(), it);
                ...
                random_choice(possible_targets, tgt_group_size_mean, target_ranks, rng, probabilities);
            } else {
                random_choice(possible_targets, tgt_group_size_mean, target_ranks, rng);
            }
...
std::vector<std::vector<int>> create_random_tgt_group_a2aV_traffic_matrix(int num_ranks, int token_size_KB_mean,
                                                                          int tgt_group_size_mean,
                                                                          int tgt_group_size_std, int num_tokens,
    ...
    std::vector<std::vector<int>> traffic_matrix(num_ranks, std::vector<int>(num_ranks, 0));
    for (int src_rank = 0; src_rank < num_ranks; src_rank++) {
...
std::vector<std::vector<int>> create_random_tgt_group_random_msg_size_a2aV_traffic_matrix(
    int num_ranks, int token_size_KB_mean, int token_size_KB_std, int tgt_group_size_mean, int tgt_group_size_std,
    int num_tokens, size_t dt_size, std::mt19937_64 &rng) {
    ...
    for (int src_rank = 0; src_rank < num_ranks; src_rank++) {
        ...
        for (int token = 0; token < num_tokens; token++) {
            int normal_sample_tgt_group_size = static_cast<int>(distribution_tgt_group_size(rng));
            int tgt_group_size;
            ...
            random_choice(possible_targets, tgt_group_size, target_ranks, rng);
            for (int i = 0; i < target_ranks.size(); i++) {
                int normal_sample_token_size = static_cast<int>(distribution_token_size(rng));
                traffic_matrix[src_rank][target_ranks[i]] += std::max(0, normal_sample_token_size) * (1000 / dt_size);
            }
        }
    }
    return traffic_matrix;
...
void print_result(std::vector<std::vector<int>> traffic_matrix, bool print_full_result) {
...
ucc_pt_generator_traffic_matrix::ucc_pt_generator_traffic_matrix(int kind, uint32_t gsize, uint32_t rank,
                                                                 ucc_datatype_t dtype, ucc_pt_op_type_t type,
                                                                 size_t nrepeats, int token_size_KB_mean_,
                                                                 int num_tokens_, int tgt_group_size_mean_,
                                                                 uint64_t seed) {
    ...
    if (kind == 0) {
        traffic_matrix = create_a2aV_traffic_matrix(comm_size, token_size_KB_mean, tgt_group_size_mean, num_tokens,
                                                    dt_size, rng_, false, bias_factor, num_hl_ranks);
    } else if (kind == 1) {
        traffic_matrix = create_a2aV_traffic_matrix(comm_size, token_size_KB_mean, tgt_group_size_mean, num_tokens,
                                                    dt_size, rng_, true, bias_factor, num_hl_ranks);
    } else if (kind == 2) {
        traffic_matrix = create_random_tgt_group_a2aV_traffic_matrix(comm_size, token_size_KB_mean, tgt_group_size_mean,
                                                                     tgt_group_size_std, num_tokens, dt_size, rng_);
    } else if (kind == 3) {
        traffic_matrix = create_random_tgt_group_random_msg_size_a2aV_traffic_matrix(
    ...
    if (traffic_matrix[0].size() != comm_size || traffic_matrix.size() != comm_size) {
        throw std::runtime_error("Matrix size (" + std::to_string(traffic_matrix[0].size()) + "x" +
                                 std::to_string(traffic_matrix.size()) + ") is not equal to comm_size*comm_size (" +
...
bool ucc_pt_generator_traffic_matrix::has_next() { return current_rep < nrep; }
...
ucc_count_t *ucc_pt_generator_traffic_matrix::get_src_counts() { return (ucc_count_t *)src_counts.data(); }

ucc_aint_t *ucc_pt_generator_traffic_matrix::get_src_displs() { return (ucc_aint_t *)src_displs.data(); }

ucc_count_t *ucc_pt_generator_traffic_matrix::get_dst_counts() { return (ucc_count_t *)dst_counts.data(); }

ucc_aint_t *ucc_pt_generator_traffic_matrix::get_dst_displs() { return (ucc_aint_t *)dst_displs.data(); }
...
void ucc_pt_generator_traffic_matrix::setup_counts_displs() {
    ...
    if (counts.size() < comm_size * comm_size) {
        throw std::runtime_error("Pattern size (" + std::to_string(counts.size()) +
                                 ") is less than comm_size*comm_size (" + std::to_string(comm_size * comm_size) + ")");
    }
