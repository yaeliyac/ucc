...
std::vector<std::vector<double>> create_a2aV_traffic_matrix(int num_ranks, int token_size_KB_mean, int tgt_group_size_mean, int num_tokens, size_t dt_size, bool add_bias=false, double bias_factor=2, int num_hl_ranks=2) {
    ...
    for (int i = 0; i < num_hl_ranks; i++) {
        bias_indices[i] = std::uniform_int_distribution<int>(0, num_ranks - 1)(generator);
    }
...
ucc_pt_generator_traffic_matrix::ucc_pt_generator_traffic_matrix(
    ...
    size_t nrepeats,
    int token_size_KB_mean_,
    int num_tokens_,
    int tgt_group_size_mean_)
{
    ...
    if (kind == 0) {
        traffic_matrix = create_a2aV_traffic_matrix(comm_size, token_size_KB_mean, tgt_group_size_mean, num_tokens, dt_size, false, bias_factor, num_hl_ranks);
    } else if (kind == 1) {
        traffic_matrix = create_a2aV_traffic_matrix(comm_size, token_size_KB_mean, tgt_group_size_mean, num_tokens, dt_size, true, bias_factor, num_hl_ranks);
    } else if (kind == 2) {
