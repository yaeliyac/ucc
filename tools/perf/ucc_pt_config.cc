...
ucc_status_t ucc_pt_config::process_args(int argc, char *argv[])
{
    ...
    static struct option long_options[] = {
        {"gen", required_argument, 0, 0}, {"seed", required_argument, 0, 0}, {0, 0, 0, 0}};
            ...
            if (strcmp(long_options[option_index].name, "gen") == 0) {
                ...
                if (gen_arg.rfind("exp:", 0) == 0) {
                    ...
                    if (at_pos != std::string::npos) {
                        ...
                        try {
                            ucc_status_t st =
                                ucc_str_to_memunits(gen_arg.substr(min_pos + 4, at_pos - (min_pos + 4)).c_str(),
                                                    (void *)&bench.gen.exp.min);
                            if (st != UCC_OK) {
                                std::cerr << "Failed to parse min value" << std::endl;
                                return st;
                            }
                            st = ucc_str_to_memunits(gen_arg.substr(max_pos + 4).c_str(), (void *)&bench.gen.exp.max);
                            if (st != UCC_OK) {
                        ...
                        try {
                            ucc_status_t st =
                                ucc_str_to_memunits(gen_arg.substr(min_pos + 4).c_str(), (void *)&bench.gen.exp.min);
                            if (st != UCC_OK) {
                                std::cerr << "Failed to parse min value" << std::endl;
                                return st;
                            }
                    ...
                    if (find_param("kind", kind_str)) {
                        ...
                        if (ks == "0" || ks.find("normal") != std::string::npos) {
                            bench.gen.matrix.kind = 0;
                        } else if (ks == "1" || ks.find("biased") != std::string::npos) {
                            bench.gen.matrix.kind = 1;
                        } else if (ks == "2" || ks.find("random_tgt_group") != std::string::npos) {
                            bench.gen.matrix.kind = 2;
                        } else if (ks == "3" || ks.find("random_tgt_group_random_msg_size") != std::string::npos) {
                            bench.gen.matrix.kind = 3;
                        } else {
                            std::cerr << "Invalid kind value in --gen matrix: accepts 0,1,2,3 or names {normal, "
                                         "biased, random_tgt_group, random_tgt_group_random_msg_size}"
                                      << std::endl;
                            return UCC_ERR_INVALID_PARAM;
                        }
                    }
                    ...
                    if (find_param("nrep", nrep_str)) {
                        try {
                            ...
                            std::cerr << "Invalid nrep value in --gen matrix" << std::endl;
                            return UCC_ERR_INVALID_PARAM;
                        }
                    }
                    ...
                    if (find_param("token_size", token_size_str)) {
                        try {
                            ...
                            std::cerr << "Invalid token_size value in --gen matrix" << std::endl;
                            return UCC_ERR_INVALID_PARAM;
                        }
                        bench.gen.matrix.token_size_KB_std = bench.gen.matrix.token_size_KB_mean;
                    }
                    ...
                    if (find_param("num_tokens", num_tokens_str)) {
                        try {
                            ...
                            std::cerr << "Invalid num_tokens value in --gen matrix" << std::endl;
                            return UCC_ERR_INVALID_PARAM;
                        }
                    }
                    ...
                    if (find_param("tgt_group_size", tgt_group_size_str)) {
                        try {
                            bench.gen.matrix.tgt_group_size_mean = std::stoull(tgt_group_size_str);
                        } catch (const std::exception &) {
                            std::cerr << "Invalid tgt_group_size value in --gen matrix" << std::endl;
                            return UCC_ERR_INVALID_PARAM;
                        }
                    }
                } else {
                    std::cerr << "Invalid value for --gen. Use exp:min=N[@max=M] or file:name=filename[@nrep=N] or "
                                 "matrix:kind=mat_kind[@nrep=N@token_size=M@num_tokens=K]"
                              << std::endl;
                    return UCC_ERR_INVALID_PARAM;
                }
